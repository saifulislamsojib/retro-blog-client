...
const BlogDetails = () => {
  ...
  const handleLike = (likes) => {
    if (loggedInUser.email) {
      ...
      fetch(`https://retro-blog-server.vercel.app/updateBlog/${id}`, {
        method: "PATCH",
      ...
      <div className="container mx-auto">
        {loading || !blogs.length ? (
          ...
          <div className="p-3 text-justify">
            ...
            <h1 className="text-center my-2 text-xl font-semibold text-purple-900">
              {title}
            </h1>
            <h3 className="mb-2 text-center text-md font-normal">
              Author:{" "}
            ...
            <h3 className="mb-2 text-center font-medium">
              Published: {date || new Date().toDateString()} -{" "}
              {time || new Date().toTimeString()}
            </h3>
            <button
              onClick={() => handleLike(likes)}
              className="block mx-auto focus:outline-none rounded-full font-medium relative text-red-400 hover:text-red-500"
            >
              <HeartIcon
                className={`h-14 w-14 inline ${
                  liked ? "fill-current" : ""
                } cursor-pointer`}
              />
              <span
            ...
            <div className="mt-5">
              {content?.split(/\n/)?.map((item, i) => (
                <p key={i} className={item === "" ? "mb-4" : "mb-1"}>
                  {item}
                </p>
              ))}
            </div>
