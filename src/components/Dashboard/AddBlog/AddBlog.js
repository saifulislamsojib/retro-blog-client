...
const AddBlog = () => {
  ...
  const submit = (data) => {
    ...
    toast.promise(
      fetch("https://retro-blog-server.vercel.app/addBlog", {
        method: "POST",
      ...
      <form onSubmit={handleSubmit(submit)} className="mt-5 md:w-11/12 mx-auto">
        <div className="text-right">
          <label
            ...
            className={
              imageUrl
                ? "bg-red-500 px-3 py-2 rounded text-gray-100 cursor-pointer hover:bg-blue-600 select-none"
                : "bg-blue-500 px-3 py-2 rounded text-gray-100 cursor-pointer hover:bg-blue-600 select-none duration-200"
            }
          >
            <UploadIcon className="h-6 w-6 inline mr-2" />
            {imageUrl ? "Image Uploaded" : "Upload Cover Image"}
          </label>
          {error.image && (
            <span className="text-red-500 block mt-2 ml-2">
              Image Is Required
            </span>
          )}
        </div>
        ...
        <input
          type="submit"
          value="Sava Blog"
          className="bg-purple-500 px-4 block ml-auto py-2 rounded text-gray-100 cursor-pointer hover:bg-blue-600 select-none duration-200 focus:ring-2 my-2"
        />
      </form>
    </div>
  );
