...
const Dashboard = () => {
  ...
  const { isAdmin, setIsAdmin, loggedInUser, adminLoaded, setAdminLoaded } =
    useContext(context);
    ...
    (async () => {
      if (!adminLoaded) {
        const res = await fetch(
          `https://retro-blog-server.vercel.app/isAdmin?email=${email}`
        );
    ...
    <div>
      {loading ? (
          ...
          <div className="col-span-4">
            <div className="flex items-center justify-between py-2 px-3 rounded-lg sticky top-0 bg-white shadow-lg">
              <button
                ...
                className="mt-1 navbar-toggler lg:hidden focus:ring-2 p-1 rounded-full focus:outline-none"
              >
        ...
        <div className="text-center">
          <Navbar />
          <h2 className="text-red-500 mt-8 text-3xl font-medium">
