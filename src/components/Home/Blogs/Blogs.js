...
const Blogs = ({ manageBlog }) => {
  ...
  const handleDelete = (_id) => {
    swal({
      ...
      if (willDelete) {
        fetch(`https://retro-blog-server.vercel.app/deleteBlog/${_id}`, {
          method: "DELETE",
          ...
          .then((result) => {
            if (result) {
              setBlogs((preBlogs) =>
                preBlogs.filter((blog) => blog._id !== _id)
              );
    ...
    <>
      {blogs.length === 0 ? (
        ...
        <div className="container mx-auto">
          <h1 className="my-5 text-center text-3xl font-semibold text-blue-800">
            Blogs
          </h1>
